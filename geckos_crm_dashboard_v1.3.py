...
if uploaded_file is not None:
    try:
        ...
        prod_cols = ['產品名稱']
        for c in ['產品類別', '開案類別', '供應商/代理商', 'CMR', 'CMR(含動靜環攤提)', '動能客戶1', '動能客戶2', '動能客戶3']:
    ...
    def render_drilldown_panel():
        ...
        if st.session_state.active_view == 'client' and st.session_state.active_target:
            ...
            with tab1:
                ...
                info_str = f"**🏷️ 產品類別**：{p_cat} ｜ **📂 開案類別**：{p_open} ｜ **🏭 供應商/代理商**：{p_sup}"
                
                if 'CMR' in df_p_schedule.columns and not df_p_schedule.empty:
                    cmr_val = df_p_schedule['CMR'].iloc[0]
                    if pd.notna(cmr_val) and str(cmr_val).strip().lower() not in ['nan', 'none', '未標示', '', '<na>']:
                        disp_val = f"{cmr_val * 100:.1f}%" if isinstance(cmr_val, (int, float)) else str(cmr_val)
                        info_str += f" ｜ **💰 CMR**：{disp_val}"
                        
                if 'CMR(含動靜環攤提)' in df_p_schedule.columns and not df_p_schedule.empty:
                    cmr_adj_val = df_p_schedule['CMR(含動靜環攤提)'].iloc[0]
                    if pd.notna(cmr_adj_val) and str(cmr_adj_val).strip().lower() not in ['nan', 'none', '未標示', '', '<na>']:
                        disp_val = f"{cmr_adj_val * 100:.1f}%" if isinstance(cmr_adj_val, (int, float)) else str(cmr_adj_val)
                        info_str += f" ｜ **💰 CMR(含動靜環攤提)**：{disp_val}"
    ...
    # 情境 B：傳統雙欄模式
        ...
        st.caption("💡 操作指南：在左側網格卡片尋找目標並點選「🔍 檢視分析」，系統將自動隱藏左側選單，進入【全螢幕專注模式】顯示深度圖表！")
